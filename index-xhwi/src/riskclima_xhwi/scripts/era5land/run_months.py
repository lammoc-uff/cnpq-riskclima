...
from riskclima_xhwi.config.settings import ERA5LandSettings, ExistingFilePolicy
from riskclima_xhwi.io.writers import (
    build_monthly_output_dataset,
    normalize_months,
    write_monthly_netcdf,
)
from riskclima_xhwi.pipeline.monthly_pipeline import compute_era5land_monthly_xhwi_torch
...
from riskclima_xhwi.scripts.era5land.common import add_common_arguments, settings_from_args

LOGGER = logging.getLogger(__name__)


def build_parser() -> argparse.ArgumentParser:
    """Build the ERA5-Land monthly parser."""
    parser = argparse.ArgumentParser(description="Compute configured ERA5-Land months.")
...
def settings_with_args(
    ...
    for field in ("months_to_run", "calibration_policy", "part_existing_policy"):
        ...
        if value is not None:
            updates[field] = value
    return ERA5LandSettings.model_validate(settings.model_dump() | updates)


def run(settings: ERA5LandSettings, months: Sequence[int]) -> list[Path]:
    """Compute and write selected ERA5-Land calendar months."""
    ...
    if settings.part_existing_policy is ExistingFilePolicy.SKIP and not pending:
        return outputs
    dataset = compute_era5land_monthly_xhwi_torch(
        settings.zarr_url,
    ...
    for month, output in zip(selected_months, outputs, strict=True):
        ...
        if monthly.sizes.get("time", 0) == 0:
            LOGGER.warning("No ERA5-Land output generated for month %02d", month)
            continue
...
def main(argv: Sequence[str] | None = None) -> None:
    """Run ERA5-Land monthly processing."""
    configured = ERA5LandSettings()
    args = build_parser().parse_args(argv)
    settings = settings_with_args(args, configured)
    configure_logging(settings)
    run(settings, normalize_months(settings.months_to_run))
